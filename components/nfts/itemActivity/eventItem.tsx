...
const EventItem = (props: { event: any }) => {
  return (
    <div className={style.eventItem}>
      ...
      <div className={`${style.eventPrice} flex-[2]`}>
        ...
        />
        <div className={style.eventPriceValue}>{props.event.price}</div>
      </div>
      <div className={`${style.accent} flex-[3]`}>{props.event.from}</div>
      <div className={`${style.accent} flex-[3]`}>{props.event.to}</div>
      <div className={`${style.accent} flex-[2]`}>{props.event.date}</div>
    </div>
  )
}
